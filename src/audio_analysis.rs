...
impl AudioAssertions {
    ...
    pub fn assert_latency_measured(measured: usize, expected: usize, tolerance: usize) {
        ...
        assert!(
            ...
            measured, expected, diff, tolerance
        );
    }
    
    /// 断言THD在预期范围内
    pub fn assert_thd_below(samples: &[f32], max_thd_db: f32, sample_rate: f32, fundamental_freq: f32) {
...
mod tests {
    ...
    fn test_stereo_correlation() {
        ...
        assert!(corr > 0.99, "Same signals should have high correlation, got {}", corr);
        
        // 反相信号 = 应该高度负相关
        let right_inv: Vec<f32> = left.iter().map(|&s| -s).collect();
        let corr_inv = measure_stereo_correlation(&left, &right_inv);
        assert!(corr_inv < -0.99, "Inverted signals should have high negative correlation, got {}", corr_inv);
    ...
    fn test_spectrum_analyzer() {
        ...
        assert!(max_db - min_db > 10.0, "Spectrum should have some variation");
    }
