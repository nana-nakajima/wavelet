    ...
    pub fn process(&mut self, input: f32) -> f32 {
        ...
        match self.config.mode {
            ...
            RingModulatorMode::Lfo => {
                ...
                let modulated_freq =
                    self.config.osc_frequency * (1.0 + lfo_value * self.config.lfo_depth);

                // 使用调制后的频率重新计算相位
                let modulated_increment = 2.0 * PI * modulated_freq / self.sample_rate;
                let modulated_phase = (self.carrier_increment * self.sample_rate / (2.0 * PI))
                    .atan2(1.0)
...
impl StereoRingModulator {
    ...
    pub fn set_config(&mut self, config: RingModulatorConfig) {
        ...
        config_r.osc_frequency =
            config.osc_frequency * (1.0 + (self.stereo_phase / (2.0 * PI)).sin() * 0.01); // 微调频率
...
mod tests {
    ...
    fn test_ring_modulator_modes() {
        ...
        assert!(
            rms_osc > 0.01,
            "Oscillator mode should produce valid output"
        );
