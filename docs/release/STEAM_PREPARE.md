# 🚀 WAVELET Steam发布准备

**项目**: WAVELET - Abstract Sound Synthesizer
**状态**: 🔧 进行中 - 第1周
**开始时间**: 2026-02-03 03:45
**最后更新**: 2026-02-03 04:45
**目标**: Steam商店发布准备

---

## 📋 Steam商店要求清单

### 1️⃣ 必需图形素材

#### 商店展示素材 (Store Assets)
- [ ] **主图 (Header Capsule)** - 920px × 430px
  - 只需游戏Logo和艺术图
  - 展示WAVELET品牌形象

- [ ] **小图 (Small Capsule)** - 462px × 174px
  - 只需游戏Logo和艺术图
  - 用于搜索结果等小尺寸展示

- [ ] **主展示图 (Main Capsule)** - 1232px × 706px
  - 只需游戏Logo和艺术图
  - 商店页面主要展示位置

- [ ] **竖图 (Vertical Capsule)** - 748px × 896px
  - 只需游戏Logo和艺术图
  - 用于侧边栏等竖向空间

- [ ] **截图 (Screenshots)** - 至少1920px × 1080px (16:9比例)
  - 至少需要5-10张高质量截图
  - 展示不同功能和音色
  - 建议包含:
    - 主界面截图
    - AI旋律生成演示
    - 预设切换效果
    - 社区功能展示
    - 可视化效果展示

#### 社区和客户端图标 (Community & Client Icons)
- [ ] **快捷图标 (Shortcut Icon)** - 256px × 256px (.ico或.png)
  - 游戏Logo或代表性图标

- [ ] **应用图标 (App Icon)** - 184px × 184px (.jpg)
  - 小尺寸游戏Logo

#### 库展示素材 (Library Assets)
- [ ] **库主图 (Library Capsule)** - 600px × 900px
  - 游戏Logo和艺术图

- [ ] **库英雄图 (Library Hero)** - 3840px × 1240px (.png)
  - 游戏艺术图

- [ ] **库Logo (Library Logo)** - 1280px宽或720px高 (.png)
  - 游戏Logo，显示在Hero图上方

- [ ] **库头图 (Library Header Capsule)** - 920px × 430px
  - 游戏Logo和艺术图

### 2️⃣ 宣传视频 (Trailer)

#### 技术要求
- **分辨率**: 最高1920×1080
- **帧率**: 30/29.97 或 60/59.94 fps
- **码率**: 5000+ Kbps
- **格式**: .mov, .wmv, 或 .mp4
- **编码**: H.264视频 + AAC音频
- **比例**: 16:9最佳

#### 视频内容建议
**开场几秒抓住用户** (用户可能只有10秒印象)
- 开场展示WAVELET的UI和视觉效果
- 快速展示"30秒上手"的简单操作
- 突出AI辅助创作的独特功能

**主要内容** (60-90秒)
- 🎹 **UI展示** - 现代化界面和主题切换
- 🤖 **AI功能** - 旋律/和弦/节奏生成演示
- 🎨 **视觉效果** - 实时波形和频谱分析
- 👥 **社区功能** - 预设分享和创作挑战
- 🎵 **音色展示** - 各种预设效果

**分类标签**: Gameplay (主要展示游戏性/操作性)

#### 缩略图
- **海报图**: 600px × 380px
- **缩略图**: 232px × 130px

### 3️⃣ 商店页面内容

#### 标题和标签
- **标题**: WAVELET - Abstract Sound Synthesizer
- **简短描述**: 让每个人都能创造音乐
- **Steam标签**:
  - Audio Production
  - Music
  - Creative
  - Education
  - Casual
  - Synthesizer

#### 商店页面文案

**About This Game** (300-500字)

# 🎹 WAVELET - 让音乐创作变得简单有趣

还在为复杂的模块合成器头疼？WAVELET让你**30秒内就能创造出属于自己的音乐**！

🌟 **为什么选择WAVELET？**

✨ **零门槛上手** - 无需学习复杂连线，30秒发出第一个声音
🎨 **视觉盛宴** - 3种主题切换，实时波形像心电图一样跳动
🤖 **AI你的音乐搭档** - 选择风格，一键生成旋律、和弦、节奏
🎹 **专业级音色** - 50+预设，从贝斯到镲片，应有尽有
👥 **社区大家庭** - 发现他人的酷炫音色，参与每周创作挑战

🎮 **谁适合用WAVELET？**
- 🎵 **音乐新手** - 想做音乐但不知从何开始
- 🎼 **创作者** - 需要快速找到好听的音色灵感
- 🎮 **游戏开发者** - 为游戏制作独特音效
- 📚 **音乐老师** - 让学生轻松理解声音合成原理

💡 **强大功能一览**
- 🎛️ **声音引擎** - 6种波形、5种滤波器、ADSR包络、12种效果器
- 🤖 **AI作曲家** - 旋律生成(6种风格)、和弦进行(8种风格)、节奏模式(12种风格)
- 👥 **社区功能** - 预设分享、创作挑战、排行榜
- 📊 **可视化** - 实时波形、频谱分析、主题动画

🔓 **免费使用** - 基础功能完全免费，专业版解锁全部AI功能和云端同步。

---

**About This Game (English)**

# 🎹 WAVELET - Music Creation Made Simple & Fun

Too intimidated by complex modular synths? WAVELET lets you **create your own music in just 30 seconds**!

🌟 **Why WAVELET?**

✨ **Zero Learning Curve** - Make your first sound in 30 seconds, no complex patching
🎨 **Stunning Visuals** - 3 themes, real-time waveforms that pulse like an ECG
🤖 **AI Your Music Buddy** - Pick a style, generate melody, chords, and rhythms instantly
🎹 **Pro-Quality Sounds** - 50+ presets covering bass, pads, leads, and more
👥 **Community Hub** - Discover cool sounds from others, join weekly challenges

🎮 **Who Is WAVELET For?**
- 🎵 **Music Beginners** - Want to make music but don't know where to start
- 🎼 **Creators** - Need quick access to great-sounding presets
- 🎮 **Game Developers** - Create unique sound effects for games
- 📚 **Music Educators** - Help students understand sound synthesis easily

💡 **Powerful Features**
- 🎛️ **Sound Engine** - 6 waveforms, 5 filters, ADSR envelope, 12 effects
- 🤖 **AI Composer** - Melody (6 styles), Chord Progressions (8 styles), Rhythms (12 styles)
- 👥 **Community** - Preset sharing, creative challenges, leaderboards
- 📊 **Visualization** - Real-time waveforms, spectrum analysis, theme animations

🔓 **Free to Start** - Core features are completely free. Pro version unlocks all AI features and cloud sync.

#### 特色截图说明
每张截图需要添加简短说明 (1-2句):
1. "现代化UI设计 - 30秒内发出第一个声音"
2. "AI旋律生成器 - 选择风格和音阶，一键生成"
3. "实时波形可视化 - 频谱分析像彩虹一样"
4. "社区预设分享 - 发现其他创作者的酷炫音色"
5. "每周创作挑战 - 与全球音乐爱好者一起创作"

### 4️⃣ 定价策略

#### 选项A: 免费 + 内购
- **基础版**: 免费
  - 完整音频引擎
  - 50个基础预设
  - MIDI键盘支持
  - 基础教程
- **专业版 ($19.99)**
  - 100+完整预设库
  - AI旋律/和弦/节奏生成
  - 云端预设同步
  - 社区功能
  - 优先更新

#### 选项B: 免费增值 (Freemium)
- **永久免费**: 核心功能
- **可选DLC**:
  - Electronic Essentials ($4.99)
  - Cinematic Sounds ($4.99)
  - Retro 80s ($4.99)

### 5️⃣ 本地化支持

**建议支持语言**:
- English (英语)
- 简体中文 (Chinese Simplified)
- 日本語 (Japanese)
- Español (Spanish)
- Deutsch (German)
- Français (French)

### 6️⃣ 系统要求

**最低配置**:
- OS: Windows 10/11, macOS 11+, Ubuntu 20.04+
- Processor: Any modern 64-bit processor
- Memory: 4GB RAM
- Storage: 500MB available space
- Additional: MIDI keyboard optional

**推荐配置**:
- OS: Windows 11, macOS 14+, Ubuntu 22.04+
- Processor: Modern multi-core processor
- Memory: 8GB RAM
- Storage: 1GB SSD
- Additional: MIDI keyboard recommended

---

## 🎬 宣传素材计划

### 宣传视频脚本

**时长**: 60-90秒

**开场** (0:00 - 0:10)
🎵 轻快音乐 + WAVELET Logo动画
📝 文字: "让每个人都能创造音乐"

**功能展示** (0:10 - 0:50)
🎹 UI快速展示 (5秒)
- 旋钮、按钮、波形可视化
- 主题切换效果

🤖 AI功能演示 (15秒)
- 选择风格 → 生成旋律
- 选择和弦进行 → 播放
- 选择节奏模式 → 生成鼓点

🎨 视觉效果展示 (10秒)
- 波形像心电图一样跳动
- 频谱分析像彩虹

👥 社区功能展示 (10秒)
- 预设分享
- 创作挑战排行榜

**结尾** (0:50 - 0:60)
💰 定价信息
📅 发布日期
🔗 steam link
📝 文字: "WAVELET - 让每个人都能创造音乐"

### 游戏截图计划

需要制作 **10-12张高质量截图**:

1. **主界面 (1920×1080)**
   - 展示完整UI
   - 标注主要功能区

2. **AI旋律生成界面**
   - 展示风格选择和生成按钮
   - 显示生成的旋律

3. **预设浏览器**
   - 分类展示各种预设
   - 快速切换效果

4. **主题对比 (3张)**
   - Dark主题
   - Retro主题
   - Cyber主题

5. **波形可视化**
   - 实时音频采样可视化
   - 频谱分析效果

6. **社区面板**
   - 预设分享界面
   - 用户资料

7. **创作挑战**
   - 排行榜展示
   - 倒计时计时器

8. **演奏演示**
   - MIDI键盘演奏
   - 视觉效果同步

9. **效果器界面**
   - Reverb、Delay等效果展示
   - 参数调节

10. **教程引导**
    - 首次使用引导界面
    - 简单操作提示

**详细截图说明**: [点击查看 SCREENSHOT_GUIDE.md](./SCREENSHOT_GUIDE.md)

---

## 📸 截图自动化方案

### Godot截图方法

Godot 4内置了截图功能，可以通过以下方式捕获高质量截图：

```gdscript
# 在Godot中捕获截图
func capture_screenshot(filename: String):
    var image = get_viewport().get_texture().get_image()
    image.save_png("res://screenshots/" + filename + ".png")
```

### 自动化截图脚本

创建一个Python脚本来自动化截图捕获流程：

```python
#!/usr/bin/env python3
"""
WAVELET 截图自动化脚本
用于捕获Steam商店所需的各类素材
"""

import subprocess
import os
from pathlib import Path

class ScreenshotCapture:
    def __init__(self, godot_executable: str, project_path: str):
        self.godot = godot_executable
        self.project = project_path
        self.output_dir = Path("steam_assets/screenshots")
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def capture_view(self, scene: str, view_name: str, delay: float = 2.0):
        """捕获特定视图"""
        # 通过命令行参数加载特定场景并截图
        cmd = [
            self.godot,
            "--path", self.project,
            "-s", "res://tools/screenshot_tool.gd",
            "--scene", scene,
            "--output", str(self.output_dir / f"{view_name}.png"),
            "--delay", str(delay)
        ]
        subprocess.run(cmd, check=True)

    def capture_all_views(self):
        """捕获所有需要的视图"""
        views = [
            ("res://scenes/main.tscn", "main_interface"),
            ("res://scenes/ai_melody.tscn", "ai_melody_generator"),
            ("res://scenes/presets.tscn", "preset_browser"),
            ("res://scenes/community.tscn", "community_panel"),
            ("res://scenes/challenges.tscn", "challenges"),
            ("res://scenes/visualizer.tscn", "waveform_visualizer"),
        ]

        for scene, name in views:
            self.capture_view(scene, name)
            print(f"✓ Captured: {name}")

# 使用示例
if __name__ == "__main__":
    capture = ScreenshotCapture(
        godot_executable="/Applications/Godot.app/Contents/MacOS/Godot",
        project_path="/path/to/wavelet/godot"
    )
    capture.capture_all_views()
```

### Godot截图工具脚本

在Godot项目中创建 `res://tools/screenshot_tool.gd`：

```gdscript
extends SceneTree

var output_path: String = ""
var scene_path: String = ""
var delay_time: float = 2.0

func _init():
    parse_args()

func parse_args():
    var args = OS.get_cmdline_args()
    for i in range(args.size()):
        if args[i] == "--scene" and i + 1 < args.size():
            scene_path = args[i + 1]
        elif args[i] == "--output" and i + 1 < args.size():
            output_path = args[i + 1]
        elif args[i] == "--delay" and i + 1 < args.size():
            delay_time = float(args[i + 1])

func _ready():
    if scene_path == "":
        print("Error: No scene specified")
        quit(1)
        return

    # 切换到指定场景
    change_scene_to_file(scene_path)

    # 等待UI加载
    await get_tree().create_timer(delay_time).timeout

    # 捕获截图
    capture_screenshot()

    quit(0)

func capture_screenshot():
    var viewport = get_viewport()
    var image = viewport.get_texture().get_image()

    if output_path == "":
        output_path = "screenshot.png"

    var err = image.save_png(output_path)
    if err == OK:
        print(f"Screenshot saved to: {output_path}")
    else:
        print(f"Error saving screenshot: {err}")
```

### 需要捕获的截图清单

| 截图名称 | 场景文件 | 说明 |
|---------|---------|------|
| `main_interface` | `main.tscn` | 主界面全貌 |
| `ai_melody_generator` | `ai_melody.tscn` | AI旋律生成演示 |
| `preset_browser` | `presets.tscn` | 预设浏览器 |
| `theme_dark` | `main.tscn` | Dark主题 |
| `theme_retro` | `main.tscn` | Retro主题 |
| `theme_cyber` | `main.tscn` | Cyber主题 |
| `waveform_visualizer` | `visualizer.tscn` | 实时波形 |
| `community_panel` | `community.tscn` | 社区分享界面 |
| `challenges` | `challenges.tscn` | 创作挑战 |
| `chord_progression` | `ai_chords.tscn` | AI和弦进行 |

### 截图尺寸要求

- **主截图**: 1920×1080 (16:9比例)
- **竖图备用**: 1080×1920
- **缩略图**: 640×360
- **分辨率**: 至少72 DPI，建议150+ DPI用于打印

---

## 📦 打包准备

### 跨平台打包架构

WAVELET由两部分组成，需要分别打包：

```
┌─────────────────────────────────────────────────────────┐
│                    WAVELET 打包架构                      │
├─────────────────────────────────────────────────────────┤
│  Rust音频引擎 (.dll/.dylib/.so)                         │
│  - Godot GDExtension 模块                               │
│  - 包含所有音频处理逻辑                                  │
├─────────────────────────────────────────────────────────┤
│  Godot 4 前端 (.pck + 可执行文件)                       │
│  - UI渲染层                                             │
│  - 用户交互                                             │
│  - 预设管理                                             │
└─────────────────────────────────────────────────────────┘
```

### Windows 打包

```bash
# 1. 编译Rust后端 (Windows)
cargo build --release --target x86_64-pc-windows-msvc

# 2. Godot导出
# 使用Godot编辑器: Project → Export → Windows
# 配置:
# - Export Path: wavelet_windows/wavelet.exe
# - Include .pck file: true
# - Extension support: true

# 3. 目录结构
wavelet_windows/
├── wavelet.exe          # Godot可执行文件
├── wavelet.pck          # 游戏资源包
├── wavelet_compat.dll   # Godot兼容层
├── libwavelet.dll       # Rust音频引擎
├── gdextension.dll      # GDExtension
└── steam_appid.txt      # Steam AppID (发布时)
```

**Windows 图标要求**:
- `favicon.ico` - 256×256 (主要图标)
- `icon_16.png` - 16×16 (任务栏)
- `icon_32.png` - 32×32 (窗口)
- `icon_48.png` - 48×48 (桌面)
- `icon_256.png` - 256×256 (高DPI)

### macOS 打包

```bash
# 1. 编译Rust后端 (Apple Silicon)
cargo build --release --target aarch64-apple-darwin

# 2. 编译Rust后端 (Intel)
cargo build --release --target x86_64-apple-darwin

# 3. Godot导出 (通用二进制)
# Project → Export → macOS
# Export With: Default (支持Apple Silicon + Intel)
# 签名: Development证书 (测试) / Distribution证书 (发布)

# 4. 创建DMG
hdiutil create -volname "WAVELET" \
    -srcfolder wavelet.app \
    -ov -format UDZO wavelet.dmg
```

**macOS 图标要求**:
- `icon_1024.png` - 1024×1024 (App Store)
- `.icns` 文件 - 多种尺寸

**代码签名** (发布前):
```bash
# 签名Godot可执行文件
codesign --sign "Developer ID Application: Your Name" \
    --timestamp \
    --entitlements wavelet.entitlements \
    wavelet.app/Contents/MacOS/wavelet

# 公证 (Notarization)
xcrun notarytool submit wavelet.zip \
    --apple-api-key your_api_key \
    --apple-api-id your_api_id \
    --wait
```

### Linux 打包

```bash
# 1. 编译Rust后端
cargo build --release --target x86_64-unknown-linux-gnu

# 2. Godot导出
# Project → Export → Linux
# Export Path: wavelet_linux/wavelet

# 3. 创建AppImage
wget https://github.com/AppImage/AppImageKit/releases/download/continuous/appimagetool-x86_64.AppImage
chmod +x appimagetool-x86_64.AppImage

# 目录结构
wavelet_x86_64/
├── wavelet              # Godot可执行文件
├── wavelet.pck          # 游戏资源包
├── libwavelet.so        # Rust音频引擎
├── gdextension.so       # GDExtension
└── wavelet.desktop      # 桌面快捷方式

# 4. 打包
./appimagetool-x86_64.AppImage wavelet_x86_64/
```

**Linux 图标**:
- `wavelet.svg` - 矢量图标 (推荐)
- `wavelet_256.png` - 256×256 PNG

### 统一打包脚本

创建 `wavelet/package.sh` 自动化打包流程：

```bash
#!/bin/bash
set -e

echo "🎵 WAVELET Packaging Script"
echo "=========================="

# 颜色输出
GREEN='\033[0;32m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

log_info() { echo -e "${BLUE}[INFO]${NC} $1"; }
log_success() { echo -e "${GREEN}[SUCCESS]${NC} $1"; }

# 配置
VERSION="2.3.0"
BUILD_DIR="build"
PACKAGE_DIR="package"

# 清理
log_info "Cleaning build directories..."
rm -rf $BUILD_DIR $PACKAGE_DIR
mkdir -p $BUILD_DIR $PACKAGE_DIR

# 1. 编译Rust
log_info "Building Rust audio engine..."
cargo build --release

# 2. 复制Godot导出文件
log_info "Copying Godot exports..."
cp -r godot/export/* $BUILD_DIR/

# 3. 复制Rust库
log_info "Copying Rust libraries..."
case "$(uname -s)" in
    Linux*)
        cp target/release/libwavelet.so $BUILD_DIR/
        cp target/release/libgdextension.so $BUILD_DIR/
        ;;
    Darwin*)
        cp target/release/libwavelet.dylib $BUILD_DIR/
        cp target/release/libgdextension.dylib $BUILD_DIR/
        ;;
    MINGW*|CYGWIN*|MSYS*)
        cp target/release/wavelet.dll $BUILD_DIR/
        cp target/release/gdextension.dll $BUILD_DIR/
        ;;
esac

# 4. 平台特定打包
case "$(uname -s)" in
    Linux*)
        log_info "Creating AppImage..."
        # AppImage打包逻辑
        ;;
    Darwin*)
        log_info "Creating DMG..."
        # DMG打包逻辑
        ;;
    MINGW*|CYGWIN*|MSYS*)
        log_info "Creating ZIP..."
        # ZIP打包逻辑
        ;;
esac

log_success "Packaging complete!"
log_info "Output: $PACKAGE_DIR/"
```

### Steam发布配置

**steam_appid.txt**:
```
# Steam App ID
# 在Steamworks创建游戏后获取实际AppID
# 测试ID: 480 (Steamworks Test App)
480
```

**Steamworks 配置**:
- ✅ Steam App ID: 待获取
- ✅ 构建版本: v2.3.0
- ✅ Beta分支: default (公测)
- 需要配置: Steamworks SDK集成

### 依赖管理总结

| 组件 | 当前状态 | 打包要求 |
|------|---------|---------|
| Rust音频引擎 | ✅ 已编译 | 静态链接std避免兼容问题 |
| Godot前端 | ✅ 导出模板 | 4.x导出模板 |
| GDExtension | ✅ 绑定完成 | 与Godot版本匹配 |
| 预设资源 | ✅ 50个 | 包含在.pck中 |
| 第三方库 | 无 | Godot自带 |

**关键配置**:
- `export_presets.cfg` - Godot导出配置
- `cargo.toml` - Rust依赖和特性
- `package.sh` - 统一打包脚本

---

## 🎨 营销素材设计规范

### 品牌配色方案

#### 主要配色 (Primary Colors)

| 用途 | 颜色代码 | 示例 |
|------|---------|------|
| 品牌主色 | `#6C63FF` | 紫色 - 代表创意和科技 |
| 辅助色 | `#00D4FF` | 青色 - 代表现代感 |
| 强调色 | `#FF6B9D` | 粉红 - 代表活力 |
| 背景深 | `#1A1A2E` | 深蓝黑 - 沉稳专业 |
| 背景浅 | `#F8F9FA` | 纯白 - 简洁明亮 |

#### 主题配色 (Theme Colors)

**Dark主题**:
- 背景: `#1A1A2E`
- 面板: `#16213E`
- 旋钮: `#0F3460`
- 高亮: `#E94560`
- 文字: `#E0E0E0`

**Retro主题**:
- 背景: `#2D1B0E`
- 面板: `#4A3728`
- 旋钮: `#8B6914`
- 高亮: `#FF6B35`
- 文字: `#FFD700`

**Cyber主题**:
- 背景: `#0A0A12`
- 面板: `#12121F`
- 旋钮: `#00FF88` / `#FF00FF`
- 高亮: `#00FFFF`
- 文字: `#FFFFFF`

### Logo设计要求

**Logo类型**:
- **主Logo**: WAVELET 文字 + 波形图标
- **图标版**: 仅波形图标 (用于小尺寸)

**波形图标**:
```
    🎵
   /|\
  / | \
 /  |  \
/   |   \
─────────
```

**Logo使用规范**:
- 最小尺寸: 32×32像素
- 最小间距: Logo周围10%边距
- 禁止: 拉伸、变形、改变颜色

### 字体规范

**主要字体**:
- 英文: **Inter** 或 **Roboto**
- 中文: **思源黑体 (Source Han Sans)** 或 **PingFang**
- 日文: **Noto Sans JP**

**字体大小**:
- 大标题: 32-48pt
- 小标题: 24-28pt
- 正文: 16-18pt
- 说明文字: 12-14pt

### 文案风格指南

**语气风格**:
- ✅ "让音乐创作变得简单有趣"
- ❌ "专业级音频合成解决方案"
- ✅ "30秒发出第一个声音"
- ❌ "零延迟高保真音频引擎"

**营销文案特点**:
- 简洁明了 (每句不超过15字)
- 情感共鸣 (强调"简单"、"有趣"、"创意")
- 具体数字 (30秒、50+预设、12种风格)
- 行动号召 ("立即体验"、"开始创作")

---

## 📖 常见问题 (FAQ)

### 基础问题

**Q: WAVELET是什么？**
A: WAVELET是一款现代化的抽象声音合成器，让每个人都能在30秒内创造出属于自己的音乐。

**Q: WAVELET是免费的吗？**
A: 是的！基础功能完全免费，包括50+预设和完整的音频引擎。专业版($19.99)解锁全部AI功能和云端同步。

**Q: 需要音乐基础才能使用吗？**
A: 完全不需要！WAVELET的设计理念就是"零门槛上手"。即使你是音乐新手，也能通过AI辅助和预设快速创作。

**Q: 支持哪些操作系统？**
A: 支持Windows 10/11、macOS 11+和Linux (Ubuntu 20.04+)。

**Q: 支持MIDI设备吗？**
A: 支持！WAVELET支持任何标准MIDI输入设备，包括MIDI键盘、控制器等。

### 功能问题

**Q: AI旋律生成如何工作？**
A: 选择你喜欢的风格(Pop、Jazz、EDM等)和音阶，然后点击生成。AI会自动创作4-8小节的完整旋律，支持力度变化和装饰音。

**Q: 预设可以自定义吗？**
A: 可以！所有预设都可以调节参数，并且支持保存为自己的自定义预设。

**Q: 导出的音频格式支持哪些？**
A: 支持WAV和FLAC格式，包含自动归一化和淡入淡出效果。

**Q: 社区功能如何使用？**
A: 登录后可以浏览、收藏和下载其他用户分享的预设，参与每周创作挑战，与全球音乐爱好者交流。

### 技术问题

**Q: WAVELET的系统要求是什么？**
A: 最低配置需要4GB RAM和500MB硬盘空间，推荐8GB RAM以获得最佳体验。

**Q: 为什么选择Rust作为音频引擎？**
A: Rust提供无与伦比的性能和内存安全，确保音频处理零延迟、无杂音。

**Q: 数据如何存储和同步？**
A: 本地数据存储在用户目录，云端同步使用加密存储，仅同步必要的用户数据。

### 购买与支持

**Q: 专业版包含什么？**
A: 专业版包含100+完整预设库、AI旋律/和弦/节奏生成、云端预设同步、社区功能和优先更新。

**Q: 如何购买专业版？**
A: 可以通过Steam内购或官网直接购买，支持支付宝和信用卡。

**Q: 退款政策是什么？**
A: 通过Steam购买可享受Steam标准退款政策(14天内、2小时以内)。

**Q: 遇到问题如何反馈？**
A: 可以通过Steam社区论坛、GitHub Issues或发送邮件至 support@wavelet.app

### 联系与支持

- **官网**: https://wavelet.app
- **GitHub**: https://github.com/nana-nakajima/wavelet
- **邮件**: support@wavelet.app
- **Discord**: https://discord.gg/wavelet

---

## 📅 发布计划

### 时间线

**Week 1** (2026-02-03 ~ 2026-02-09):
- [x] 📝 完善商店页面文案 ✅ (2026-02-03 04:45)
  - [x] 更具营销吸引力的中文介绍
  - [x] 完整的英文版本
  - [x] 功能亮点列表
- [x] 📸 添加截图自动化方案 ✅ (2026-02-03 04:45)
  - [x] Godot截图工具脚本
  - [x] Python自动化脚本
  - [x] 10个必拍截图清单
- [x] 📦 添加详细打包准备文档 ✅ (2026-02-03 04:45)
  - [x] 跨平台架构说明
  - [x] Windows打包配置
  - [x] macOS打包配置 (含签名/公证)
  - [x] Linux打包配置 (AppImage)
  - [x] 统一打包脚本模板
  - [x] Steamworks配置说明
- [x] 🎬 制作宣传视频 🔧 (2026-02-03 06:15)
  - [x] 创建详细分镜脚本 (TRAILER_SCRIPT.md) ✅
  - [x] 旁白文案 (TRAILER_NARRATION.md) ✅
  - [x] 截图自动化脚本 (capture_screenshots.py) ✅
  - [x] 截图详细指导文档 (SCREENSHOT_GUIDE.md) ✅ (新增 2026-02-03 06:15)
  - [ ] 录制UI画面 (待Godot导出 - 阻塞!)
  - [ ] 后期剪辑制作 (待录制完成)
- [x] 📊 竞争分析与营销洞察 ✅ (2026-02-03 07:15)
  - [x] 竞争对手深度分析 (VCV Rack, Vital, Surge, Arturia) ✅
  - [x] 市场定位矩阵 ✅
  - [x] 营销策略建议 ✅
  - [x] 成功案例参考 ✅
  - [x] 发布时间线规划 ✅
- [x] 🎨 添加营销素材设计规范 ✅ (2026-02-03 13:20)
  - [x] 品牌配色方案
  - [x] Logo设计要求
  - [x] 字体规范
  - [x] 文案风格指南
- [ ] 📸 准备截图 (待Godot导出后)
- [x] 📖 添加常见问题(FAQ) ✅ (2026-02-03 13:20)
  - [x] 常见问题列表
  - [x] 技术支持信息
  - [x] 联系方式

**Week 2** (2026-02-10 ~ 2026-02-16):
- [ ] 跨平台打包测试 (待Godot UI导出)
- [x] 修复Bug和优化性能 ✅ (18:50)
- [x] 本地化测试 ✅ (18:15)
- [x] Steamworks账号配置 ✅ (16:05)

**Week 3** (2026-02-17 ~ 2026-02-23):
- [ ] 提交审核
- [ ] 准备营销素材
- [ ] 社区预告

**Week 4** (2026-02-24 ~ 2026-03-02):
- [ ] Valve审核反馈
- [ ] 修复审核问题
- [ ] 最终发布准备

---

## 📊 成功指标

**发布首月**:
- 下载目标: 1,000
- 商店评分: 4.0+
- 用户评价: 10+

**发布首年**:
- 下载目标: 10,000
- 日活跃用户: 1,000
- 商店评分: 4.5+
- 付费转化率: 5%

---

## 🔗 相关链接

- **Steamworks文档**: https://partner.steamgames.com/doc/store
- **图形素材模板**: https://www.dropbox.com/scl/fo/cvkwbosmrimklcl9h0qko/AF5IPErKP-mQM_3YO1Dw2lA
- **项目GitHub**: https://github.com/nana-nakajima/wavelet
- **任务追踪**: TASKS_INDEX.md
- **竞争分析**: [COMPETITOR_ANALYSIS.md](./COMPETITOR_ANALYSIS.md)

---

*Made with 💕 by Nana Nakajima*
*WAVELET - 让每个人都能创造音乐*
