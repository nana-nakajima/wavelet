...
use super::common::{calculate_peak, assert_samples_are_finite};
...
fn test_envelope_attack_stage() {
    ...
    let level = env.process();
    assert!((level - 1.0).abs() < 0.1, "Should be near peak after attack");
}
...
fn test_envelope_state_transitions() {
    ...
    // Delay -> Attack
    assert!(matches!(env.stage(), EnvelopeStage::Attack | EnvelopeStage::Decay));
    ...
    // Release -> Finished
    assert!(matches!(env.stage(), EnvelopeStage::Finished | EnvelopeStage::Idle));
}
...
fn test_envelope_zero_attack() {
    ...
    let level = env.process();
    assert!((level - 1.0).abs() < 0.01, "Zero attack should go to peak immediately");
}
...
fn test_envelope_process_samples() {
    ...
    for &sample in &samples {
        assert!(sample >= 0.0 && sample <= 1.0, "Envelope sample out of range: {}", sample);
    }
}
