...
pub use saturation::{Saturation, SaturationConfig, saturate};
    ...
    fn calculate_gain_reduction(&mut self, input_db: f32) -> f32 {
        ...
        let effective_input_db = if self.knee_db > 0.0 {
            if input_db < knee_start {
                ...
                input_db + (self.threshold_db - knee_end) + 
...
impl Effect for Compressor {
    fn process(&mut self, input: f32) -> f32 {
        ...
        if target_reduction < self.gain_reduction {
            ...
            self.gain_reduction = attack_coef * self.gain_reduction + (1.0 - attack_coef) * target_reduction;
        } else {
            // Release - increase gain slowly
            self.gain_reduction = release_coef * self.gain_reduction + (1.0 - release_coef) * target_reduction;
        }
...
mod tests {
    ...
    fn test_effect_processor_bypass() {
        ...
        for &effect_type in &[EffectType::Delay, EffectType::Reverb, 
            ...
            assert!(processed.abs() <= 1.0, "Processing failed for {:?}", effect_type);
        }
    }
}
