    ...
    fn process_stutter(&mut self) -> f32 {
        ...
        if self.read_pos >= length as f32 {
            ...
            if self.config.randomization > 0.0 {
                ...
                let random_offset = (self.random() * self.config.randomization * max_start as f32) as usize;
...
mod tests {
    ...
    fn test_freeze_speed_effect() {
        ...
        // 两种速度都应该产生有效输出
        assert!(rms_half > 0.0, "Speed 0.5x should produce output, RMS={}", rms_half);
        assert!(rms_double > 0.0, "Speed 2.0x should produce output, RMS={}", rms_double);
    }
