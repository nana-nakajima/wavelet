...
use wavelet::oscillator::{
    calculate_phase_increment, frequency_to_midi, midi_to_frequency, Oscillator, OscillatorConfig,
    OversampleFactor, Waveform,
};
...
fn test_midi_to_frequency_a4() {
    ...
    assert!(
        ...
        "A4 frequency: {} Hz (expected 440.0)",
        freq
    );
}
...
fn test_midi_to_frequency_c4() {
    ...
    assert!(
        (freq - 261.63).abs() < 0.5,
        "C4 frequency: {} Hz (expected ~261.63)",
        freq
    );
}
...
fn test_midi_to_frequency_c5() {
    ...
    assert!(
        (freq - 523.25).abs() < 0.5,
        "C5 frequency: {} Hz (expected ~523.25)",
        freq
    );
}
...
fn test_midi_to_frequency_high_note() {
    ...
    assert!(
        freq > 4000.0 && freq < 20000.0,
        "High note frequency: {} Hz",
        freq
    );
}
...
fn test_amplitude_clamping() {
    ...
    assert!(
        peak <= 1.0,
        "Amplitude should be clamped to 1.0: got peak {}",
        peak
    );
}
...
fn test_all_waveforms_produce_audio() {
    ...
    for waveform in [
        ...
        assert!(
            rms > 0.01,
            "Waveform {:?} should produce audio, RMS: {}",
            waveform,
            rms
        );
    }
