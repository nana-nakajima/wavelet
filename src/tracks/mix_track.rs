...
impl MixTrack {
    pub fn new(id: u8, name: &str) -> Self {
        let mut base = Track::new(id, TrackType::Mix, name);
        base.base_width_filter = None;
        MixTrack { base }
    }
}

impl TrackBehavior for MixTrack {
...
mod tests {
    ...
    #[test]
    fn test_mix_track_creation() {
        let track = MixTrack::new(16, "MIX");
        assert_eq!(track.base.id, 16);
        assert_eq!(track.base.track_type, TrackType::Mix);
        assert_eq!(track.base.name, "MIX");
        assert!(track.base.base_width_filter.is_none());
        assert_eq!(track.base.insert_fx.len(), 1);
    }

    #[test]
    fn test_mix_track_mute() {
        let mut track = MixTrack::new(16, "MIX");
