...
// Reference: Elektron Tonverk Bit Crusher
//
// Bit Crusher produces a digital retro texture by reducing the bit depth and sample rate of the signal
//
// Bit Depth Reduction:
// - Original: 24-bit or 16-bit
// - Crushed: 1-24 bit
// - Formula: output = quantize(input, bits)
//
// Sample Rate Reduction (Decimation):
// - Original: 44100 Hz
// - Reduced: 11025 Hz - 44100 Hz
// - Principle: Take every Nth sample, discard the ones in between
//
// Parameters:
// - BIT REDUCTION: Bit depth (1-24 bit)
// - SAMPLE RATE: Decimation ratio (1x - 4x)
//
// Sound characteristics:
// - Low bit depth: Produces quantization noise, rough texture
// - Decimation: Produces aliasing frequencies, metallic texture

/// Decimation mode
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DecimationMode {
    /// Simple decimation (discard intermediate samples)
    Decimate,
...
pub struct BitCrusherConfig {
    /// Bit depth (1-24 bit, 24 = lossless)
    pub bit_depth: f32,

    /// Decimation ratio (1.0 = no decimation, 4.0 = 4x decimation)
...
pub struct BitCrusher {
    ...
    /// Last processed sample (used for decimation)
    last_output: f32,

    /// Decimation interval
    decimation_interval: usize,

    /// Output gain linear value
    output_gain: f32,
...
impl BitCrusher {
    /// Create a new Bit Crusher
    pub fn new() -> Self {
        Self::new_with_sample_rate(44100.0)
    }

    /// Create a Bit Crusher with a specific sample rate
    ...
    fn quantize(&self, input: f32) -> f32 {
        ...
        if self.quantize_step >= 1.0 {
            // 1-2 bit: Use sign function
            // sign(input) * step/2 for 1-bit
            clamped.signum() * self.quantize_step * 0.5
        } else {
            // 3-24 bit: Standard quantization
    ...
    pub fn process(&mut self, input: f32) -> f32 {
        ...
        let processed = match (is_decimation_point, self.config.decimation_mode) {
            ...
            (false, DecimationMode::Linear) => {
                // Linear interpolation (simplified: return previous value)
                self.last_output
            }
        };
