...
impl Sample {
    ...
    pub fn is_looping(&self) -> bool {
        self.info.loop_info.as_ref().map(|l| l.mode != LoopMode::OneShot).unwrap_or(false)
    }
...
impl Sampler {
    ...
    pub fn seek(&mut self, position: f64) {
        if let Some(ref sample) = self.sample {
            self.position = (position * sample.info.length as f64).clamp(0.0, sample.info.length as f64 - 1.0);
        }
    }
    ...
    pub fn process(&mut self) -> (f32, f32) {
        ...
        if let Some(ref sample) = self.sample {
            ...
            (left * self.volume * self.fade_gain, right * self.volume * self.fade_gain)
        } else {
            (0.0, 0.0)
        }
    }
    ...
    fn get_sample_at(&self, index: usize) -> (f32, f32) {
        if let Some(ref sample) = self.sample {
            ...
            let right = sample.data_stereo.as_ref()
                .map(|s| s[idx])
                .unwrap_or(left);
            (left, right)
        } else {
...
mod tests {
    ...
    fn test_auto_slicer() {
        ...
        assert!(slices.len() >= 1, "Expected at least 1 slice, got {}", slices.len());
    }
