...
impl Default for FilterBankConfig {
    fn default() -> Self {
        Self {
            filters: [
                FilterBandConfig { filter_type: FilterBankType::LowShelf, frequency: 31.5, gain_db: 0.0, q: 0.7 },
                FilterBandConfig { filter_type: FilterBankType::Peak, frequency: 63.0, gain_db: 0.0, q: 1.0 },
                FilterBandConfig { filter_type: FilterBankType::Peak, frequency: 125.0, gain_db: 0.0, q: 1.0 },
                FilterBandConfig { filter_type: FilterBankType::Peak, frequency: 250.0, gain_db: 0.0, q: 1.0 },
                FilterBandConfig { filter_type: FilterBankType::Peak, frequency: 500.0, gain_db: 0.0, q: 1.0 },
                FilterBandConfig { filter_type: FilterBankType::Peak, frequency: 1000.0, gain_db: 0.0, q: 1.0 },
                FilterBandConfig { filter_type: FilterBankType::Peak, frequency: 2000.0, gain_db: 0.0, q: 1.0 },
                FilterBandConfig { filter_type: FilterBankType::HighShelf, frequency: 4000.0, gain_db: 0.0, q: 0.7 },
            ],
            input_gain_db: 0.0,
            output_gain_db: 0.0,
        }
...
impl BiquadFilter {
    ...
    fn configure_peak(&mut self, freq: f32, q: f32, gain_db: f32, sample_rate: f32) {
        ...
        self.b0 /= norm; self.b1 /= norm; self.b2 /= norm;
        self.a1 /= norm; self.a2 /= norm;
    }
    ...
    fn process(&mut self, input: f32) -> f32 {
        ...
        // 防止数值不稳定
        if !output.is_finite() { 0.0 } else { output }
    }
...
impl FilterBank {
    ...
    pub fn get_config(&self) -> FilterBankConfig { self.config }
    ...
    fn update_filters(&mut self) {
        for i in 0..8 { self.update_single_filter(i); }
    }
    ...
    fn update_single_filter(&mut self, index: usize) {
        ...
        match filter.filter_type {
            FilterBankType::Peak => f.configure_peak(filter.frequency, filter.q, filter.gain_db, self.sample_rate),
            FilterBankType::LowShelf => f.configure_low_shelf(filter.frequency, filter.q, filter.gain_db, self.sample_rate),
            FilterBankType::HighShelf => f.configure_high_shelf(filter.frequency, filter.q, filter.gain_db, self.sample_rate),
            FilterBankType::BandPass => f.configure_peak(filter.frequency, filter.q, 0.0, self.sample_rate),
        }
    }
    ...
    pub fn process(&mut self, input: f32) -> f32 {
        ...
        let mut sum = 0.0;
        for filter in &mut self.filters { sum += filter.process(input); }
    ...
    pub fn reset(&mut self) { for filter in &mut self.filters { filter.reset(); } }
    pub fn get_latency(&self) -> usize { 0 }
}
...
mod tests {
    ...
    fn test_filter_bank_zero_gain() {
        let input: Vec<f32> = (0..44100).take(22050).map(|i| {
        ...
        assert!(diff < 0.1, "Zero gain should preserve signal, RMS diff = {}", diff);
    }
    ...
    fn test_filter_bank_silence() {
        ...
        assert!(output_rms < 0.0001, "Silence input should produce silence output");
    }
    ...
    fn test_filter_bank_latency() {
        ...
        input[0] = 1.0;
        let output: Vec<f32> = input.iter().map(|&s| bank.process(s)).collect();
        let max_idx = output.iter().enumerate().max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap()).map(|(i, _)| i).unwrap();
    ...
    fn test_filter_bank_set_filter() {
        ...
        bank.set_filter(0, config);
        assert_eq!(bank.config.filters[0].filter_type, FilterBankType::HighShelf);
    }
    ...
    fn test_filter_bank_process() {
        ...
        for _ in 0..1000 {
            ...
            assert!(output.abs() <= 500.0, "Output should be bounded, got {}", output);
        }
    }
    ...
    fn test_filter_bank_reset() {
        let mut bank = FilterBank::new_with_sample_rate(44100.0);
        for _ in 0..100 { bank.process(0.5); }
        bank.reset();
        let output = bank.process(0.5);
        assert!(output.is_finite());
    }
    ...
    fn test_filter_bank_all_types() {
        ...
        for (i, &freq) in freqs.iter().enumerate() {
            for filter_type in [FilterBankType::LowShelf, FilterBankType::Peak, FilterBankType::HighShelf] {
                let mut bank = FilterBank::new_with_sample_rate(44100.0);
                let config = FilterBandConfig { filter_type, frequency: freq, gain_db: 3.0, q: 1.0 };
                bank.set_filter(i, config);
                let output = bank.process(0.5);
                assert!(output.is_finite());
            }
    ...
    fn test_filter_bank_gain_bounds() {
        ...
        bank.set_filter(0, config);
        assert!(bank.config.filters[0].gain_db <= 12.0, "Gain should be clamped to 12.0");
