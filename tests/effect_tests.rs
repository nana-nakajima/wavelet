...
use wavelet::effects::{
    ...
    Compressor, Delay, Distortion, Effect, EffectConfig, EffectProcessor, EffectType, Reverb,
};
...
fn test_delay_reset() {
    ...
    assert!(
        (output - 0.5).abs() < 0.01,
        "After reset, delay should only pass input"
    );
}
...
fn test_all_effects_no_nan_or_inf() {
    ...
    for (name, mut effect) in effects {
        for _ in 0..10000 {
            ...
            assert!(output.is_finite(), "NaN or Inf detected in {} effect", name);
        }
    }
}
