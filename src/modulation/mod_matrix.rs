...
impl ModulationMatrix {
    ...
    pub fn add_connection(&mut self, connection: ModulationConnection) -> Result<usize, ModulationMatrixError> {
    ...
    pub fn add_connection_from_config(&mut self, config: ModulationConnectionConfig) -> Result<usize, ModulationMatrixError> {
    ...
    pub fn connections_for_source(&self, source_type: ModulationSourceType, source_id: u8) -> Vec<&ModulationConnection> {
        let key = (source_type, source_id);
        self.source_map
            .get(&key)
            .map(|indices| indices.iter().filter_map(|&i| self.connections.get(i)).collect())
            .unwrap_or_default()
    }
    
    /// Gets connections for a specific target
    pub fn connections_for_target(&self, target_type: ModulationTargetType, target_id: u8) -> Vec<&ModulationConnection> {
    ...
    pub fn update_from_source(&mut self, source_type: ModulationSourceType, source_id: u8, value: f32) {
        if !self.enabled {
            return;
        }
    ...
    pub fn total_modulation_for_target(&self, target_type: ModulationTargetType, target_id: u8) -> f32 {
        let key = (target_type, target_id);
        self.target_map
            .get(&key)
            .map(|indices| {
    ...
    pub fn process(&mut self, source_values: &HashMap<(ModulationSourceType, u8), f32>) -> HashMap<(ModulationTargetType, u8), f32> {
        if !self.enabled {
            return HashMap::new();
        }
...
mod tests {
    ...
    fn test_modulation_matrix_max_connections() {
        ...
        let conn = ModulationConnection::default_lfo_filter();
        assert_eq!(matrix.add_connection(conn), Err(ModulationMatrixError::MaxConnectionsReached));
    }
    ...
    fn test_modulation_matrix_remove_connection() {
        ...
        matrix.add_connection(ModulationConnection::default_lfo_filter()).unwrap();
        assert_eq!(matrix.connection_count(), 1);
        ...
        // Remove non-existent
        assert_eq!(matrix.remove_connection(0), Err(ModulationMatrixError::InvalidIndex));
    }
    ...
    fn test_modulation_matrix_process() {
        ...
        assert_eq!(output.get(&(ModulationTargetType::FilterCutoff, 0)), Some(&expected_value));
    }
