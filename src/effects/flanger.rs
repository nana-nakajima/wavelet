...
impl CombFilter {
    ...
    fn set_delay(&mut self, delay: f32) {
        ...
        self.target_delay =
            self.min_delay as f32 + (self.max_delay - self.min_delay) as f32 * delay;
    }
...
impl Flanger {
    ...
    pub fn process_stereo(&mut self, input_left: f32, input_right: f32) -> (f32, f32) {
        ...
        let delay = self.config.manual + self.config.depth * 0.5 * (1.0 + lfo_value);
        ...
        // 右声道使用反相LFO
        let delay_right = self.config.manual + self.config.depth * 0.5 * (1.0 - lfo_value);
